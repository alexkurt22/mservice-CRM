...
class PushService {
  // 1. Отправка конкретному клиенту (УЖЕ РАБОТАЕТ ИДЕАЛЬНО)
  ...
  // 2. ❗ НОВАЯ ФУНКЦИЯ: Рассылка всем сотрудникам (ДЛЯ КОМАНДНОГО ЧАТА)
  static Future<String> sendPushToAdmins(String title, String body) async {
    try {
      ...
      final payload = {
        'message': {
          'topic': 'admins', // Рация, которую слушают все сотрудники
          'notification': { 'title': title, 'body': body },
          'android': {
            'priority': 'high',
            'notification': { 'sound': 'default', 'default_vibrate_timings': true, 'default_sound': true }
